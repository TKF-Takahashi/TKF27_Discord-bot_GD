# application/model/recruit_model.py
import json
...
class RecruitModel:
    ...
    async def add_recruit(self, date_s: str, place: str, max_people: int, note: str, thread_id: int) -> int:
        """新しい募集をデータベースに追加する"""
        query = """
            INSERT INTO recruits (date_s, place, max_people, note, thread_id, participants)
            VALUES (?, ?, ?, ?, ?, ?)
        """
        # participants は初期状態では空のJSON配列（または空文字列）として保存
        # SQLiteではTEXT型にJSON文字列を保存するのが一般的
